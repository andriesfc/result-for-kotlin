...
internal class FunctionFlowControlTest {
    ...
    fun `Use 'resultCatching()' with caught handler convert expected 'Throwable' to expected error`() {
        ...
        assertThat {
            resultCatching<IOException, ErrorCaseEnum, Int>(exceptionAsErrorCase2) {
                throw IOException()
            }
    ...
    fun `Using 'thenResultCatching()' to invoke the next call in the call chain`() {
        ...
        val callChain = {
            ...
            resultCatching(parsingFailureAsErrorCase1) {
                ...
                localIsoDateString
                    ...
                    .toList().also(::println)
                    .success()
            }.thenResultCatching(dateCreationFailureAsErrorCase2) {
